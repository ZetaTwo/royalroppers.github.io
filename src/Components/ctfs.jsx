  ...
  return (
    ...
    >
      <div className="p-5">
        ...
        <img
          className="my-2 rounded-lg w-full object-none"
          style={{ height: "150px" }}
          src={logo}
          alt=""
        />
      ...
      <p
        className={`text-center ${
          place === "1"
            ? "border-2 border-yellow-300 m-5 text-gold text-xl"
            : place === "2"
            ? "border-2 border-silver-300 m-5 text-silver text-xl"
            : place === "3"
            ? "border-2 border-yellow-400 m-5 text-bronze text-xl"
            : "border-2 border-gray-400 m-5 text-white"
        }`}
      >
        {ordinal(place)} place
      </p>
...
const Ctfs = function () {
  return (
    <>
      ...
      <div className="p-4 grid justify-items-center grid-cols-1 md:grid-cols-2 md:p-6 lg:grid-cols-3 lg:p-8 xl:p-10 gap-8 m-auto max-w-6xl">
        {json.ctfs
          .filter((e) => {
            return e.place < cutoff_threshold;
          })
          .map((e, i) => {
            return (
              <CTF
